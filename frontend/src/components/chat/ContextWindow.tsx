...
type RoleType = 'all' | 'router' | 'assistant' | 'agent_tools';
...
const tooltipDescriptions = {
  ...
  roleBadge: 'The function of this model call: ROUTER (decides which model/route to use), ASSISTANT (generates chat responses), or AGENT_TOOLS (executes autonomous tool calls).',
  ...
  outputMetric: 'Total tokens GENERATED by the model in its response to this request. This is what you "pay" for the answer. Only shown if the model produced output (router doesn\'t always have output tokens).',
...
const RoleBreakdown: React.FC<{
  ...
  // When API provides prompt/completion, use those. Otherwise fall back to actual or estimated.
  const hasApiBreakdown = data.prompt > 0 || data.completion > 0;
  const totalTokens = data.actual > 0 ? data.actual : data.estimated;
        ...
        {hasApiBreakdown ? (
          <>
            <div className="role-token-item">
              <span className="role-token-label">Input:</span>
              <span className="role-token-value">{data.prompt.toLocaleString()}</span>
            </div>
            <div className="role-token-item">
              <span className="role-token-label">Output:</span>
              <span className="role-token-value">{data.completion.toLocaleString()}</span>
            </div>
          </>
        ) : (
          <>
      ...
      {expanded && (
        <div className="role-expanded-details">
          ...
          {hasApiBreakdown && (
            <>
              <div className="expanded-detail-row">
                <span className="detail-label">Input (API):</span>
                <span className="detail-value">{data.prompt.toLocaleString()}</span>
              </div>
              <div className="expanded-detail-row">
                <span className="detail-label">Output (API):</span>
                <span className="detail-value">{data.completion.toLocaleString()}</span>
              </div>
            </>
          )}
          {!hasApiBreakdown && data.estimated > 0 && (
            <div className="expanded-detail-row">
              <span className="detail-label">Pre-request Estimate:</span>
              <span className="detail-value">{data.estimated.toLocaleString()}</span>
            </div>
          )}
          {info?.last_used && (
...
const ContextWindow: React.FC<ContextWindowProps> = ({
  ...
  const getTotalTokens = () => {
    ...
    // Use actual when available (API reported), otherwise fall back to estimated
    return roleData.actual > 0 ? roleData.actual : roleData.estimated;
  };
