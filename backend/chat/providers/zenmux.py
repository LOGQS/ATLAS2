...
class Zenmux:
    ...
    def __init__(self):
        ...
        if self.api_key:
            logger.info("Zenmux client initialized successfully")
            ...
            if Config.should_init_async_clients():
                try:
                    ...
                    logger.debug("Zenmux async client initialized eagerly at startup")
                except Exception as e:
                    logger.warning(f"Failed to initialize Zenmux async client at startup: {e}")
            else:
                logger.debug("Zenmux async client initialization skipped (execution mode: %s)", Config.get_chat_execution_mode())
        else:
            logger.warning("Zenmux API key not found, provider disabled")
    ...
    def count_tokens(self, text: str, model: str) -> int:
        ...
        try:
            ...
            logger.warning(f"Zenmux tiktoken counting failed: {e}, using fallback")
            return max(1, len(text) // 4)
    ...
    def generate_text(self, prompt: str, model: str = "",
        ...
        try:
            ...
            if "choices" in result and len(result["choices"]) > 0:
                ...
                logger.warning(f"Unexpected response format from Zenmux: {result}")
                return {
                    "text": None,
                    "thoughts": None,
            ...
            logger.error(f"Zenmux API request failed: {str(e)}")
            return {
                "text": None,
                "thoughts": None,
                "error": str(e)
            ...
            logger.error(f"Unexpected error in Zenmux generate_text: {str(e)}")
            return {
                "text": None,
                "thoughts": None,
                "error": str(e)
        ...
        try:
            ...
            for line in response.iter_lines():
                if line:
                    ...
                    logger.debug(f"[ZENMUX-STREAM] Raw line: {line_str[:200]}")

                    if line_str.startswith('data:'):
                        line_str = line_str[5:]  # Skip 'data:' (no space)
                        if line_str == '[DONE]':
                            logger.debug("[ZENMUX-STREAM] Received [DONE]")
                            break

                        try:
                            chunk = json.loads(line_str)
                            logger.debug(f"[ZENMUX-STREAM] Parsed chunk: {chunk}")
                            ...
                            logger.warning(f"[ZENMUX-STREAM] Failed to parse chunk: {line_str[:200]} - Error: {e}")
                            continue
                    else:
                        logger.debug(f"[ZENMUX-STREAM] Non-data line: {line_str[:100]}")
            ...
            logger.error(f"Zenmux streaming API request failed: {str(e)}")
            yield {"type": "error", "content": str(e)}
        except Exception as e:
            logger.error(f"Unexpected error in Zenmux generate_text_stream: {str(e)}")
            yield {"type": "error", "content": str(e)}
    ...
    def _ensure_async_client(self):
        ...
        try:
            ...
            logger.info("Zenmux async client (httpx) initialized successfully")
            return self.async_client
        except ImportError:
            logger.error("httpx package not installed. Please run: pip install httpx")
            return None
        except Exception as exc:
            logger.error(f"Failed to initialize Zenmux async client: {exc}")
            return None
    ...
    async def generate_text_async(self, prompt: str, model: str = "",
        ...
        try:
            ...
            logger.error(f"Zenmux async API request failed: {str(e)}")
            return {
                "text": None,
                "thoughts": None,
                "error": str(e)
                ...
                async for line in response.aiter_lines():
                    if line:
                        if line.startswith('data:'):
                            line_str = line[5:]  # Skip 'data:' (no space)
                            if line_str == '[DONE]':
                                break
